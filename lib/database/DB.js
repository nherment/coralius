...
//                console.log('>> Creating indexes on collection ['+name+']')
  ...
  this.mapReduce = function(collection, map, reduce, options, callback) {
    if(collections[collection]) {
        collections[collection].mapReduce(map, reduce, options, callback)
...
var dbConf = {
  ...
  name      : process.env.DB_NAME || 'coral',
  user      : process.env.DB_USER || 'coral',
  password  : process.env.DB_PWD  || 'coral'
}
